...
const UnamKungFuCanvas = styled.div`
  z-index: 1;
  height: 24vh;
  width: 22vh;
  max-height: 100%;
...
export default function UnamKungFuScene() {
  ...
  useEffect(() => {
    ...
    controls.minDistance = 0.5; // e.g., zoom in to a minimum of 2 units
    controls.maxDistance = 1.5; // e.g., zoom out to a maximum of 10 units
    ...
    gltfLoader.load("/assets/unam3d/unamkungfu.glb", (gltf) => {
      model.current = gltf.scene;
      model.current.position.y = -0.13;
      model.current.position.x = -0.04;
      ...
      model.current.scale.set(0.12, 0.12, 0.12);
